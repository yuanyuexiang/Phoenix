...
> 状态:**生产已启用(测试阶段,`required` 模式);WorkBuddy 真机 OAuth 流程已验证通过,卡在 UI 状态刷新**  |  版本 V1.3  |  日期 2026-07-16
...
> 调工具 → `uploaded_by` 落库。WorkBuddy 真机验证(§5)已完成:OAuth 全链路通过,
> token 已落盘 `.credentials.v3.json`(19 次成功回调),卡在 UI 状态刷新(连接器一直「授权中…」)。
> 仍待:WorkBuddy UI bug 修复 / 规避、AS 终局选型(客户 IdP 联邦 or 沿用 Keycloak)与员工账号口径(§8)。
>
> 平台侧实现说明(§4 已全部落地):
...
- [x] 支持 MCP 授权规范的 **401 → 元数据发现 → OAuth 2.1 授权码 + PKCE** 流程 ✅ 2026-07-16 实测通过
- [~] 支持 **RFC 8707 resource 参数**(2025-06-18 及以后规范的客户端要求) ⚠️ DCR 注册成功,但 **token 的 audience 未验证**(见 §6 风险行「DCR 客户端 audience」)
- [x] 能为**每个员工**独立完成授权并保存各自 token(而非组织级共享一个) ✅ token 已存入 `.credentials.v3.json`(`mcpOAuth` 字段,按 serverUrl 索引)
- [ ] 支持 token 过期后的静默刷新 ⏳ token 已有 `expiresAt` + `refreshToken`,但 UI 卡住无法触发后续调用验证

> 主流 MCP 客户端(Claude、Cursor 等)均已实现该流程;WorkBuddy v5.2.6 已实测确认支持
> 401 → 发现 → PKCE 全链路。**唯一遗留**:OAuth 回调 `success=true` 后 UI 状态未从「授权中…」翻转为「已连接」,
> 疑似客户端 UI 状态机 bug(详见 [WorkBuddy连接器接入方案.md](WorkBuddy连接器接入方案.md) §2.4)。

## 6. 风险与降级路径

| 风险 | 应对 |
|------|------|
| ~~WorkBuddy 不支持 OAuth 流程~~ | ✅ **已排除**:2026-07-16 实测全链路通过(401 → 发现 → PKCE → token 交换 → 落盘) |
| WorkBuddy UI 状态刷新 bug | OAuth 回调 `success=true` 但 UI 卡在「授权中…」,token 已落盘。尝试重启 / 断开重连 / 反馈 WorkBuddy 团队 |
| **DCR 客户端 audience(高,被 UI bug 遮住的隐患)** | audience mapper `phoenix-mcp-audience` 只挂在预定义客户端 `workbuddy`/`phoenix-smoke` 上,**不是 realm 默认**。WorkBuddy 用 DCR 注册的客户端**拿不到 `aud=phoenix-mcp`**,而 RS(`PHX_OAUTH_AUDIENCE=phoenix-mcp`)强制校验该 aud → UI 解卡后**工具调用会 401**。此前冒烟走的是带 mapper 的 `phoenix-smoke`,验证不到 DCR 路径。**修法**:把 audience mapper 做成 realm 级默认客户端范围(`defaultDefaultClientScopes`),使 DCR 客户端也带 `aud=phoenix-mcp`;或改 RS 接受 resource URL 作为 audience。**先验证再改**:UI 解卡 / MCP Inspector 走通后看 mcp 日志有无 audience 拒绝 |
...
1. **~~联调验证(1 周)~~** ✅ 已完成(2026-07-16):WorkBuddy v5.2.6 OAuth 全链路通过,
   Keycloak 测试环境 + WorkBuddy 真机验证完成。**遗留**:UI 状态刷新 bug 待解决
2. **~~平台改造(1 周)~~** ✅ 已完成:§4 清单全部落地,冒烟客户端带 token 用例通过,CI 覆盖
3. **灰度**:生产开启 `PHX_OAUTH_ISSUER`,先允许"有 token 记身份、无 token 放行"的
   过渡模式观察一周,再切强制
4. **收尾**:管理后台上线操作人展示与审计查询;更新专家发布包与接入指南;
   解决 WorkBuddy UI 状态 bug 后推送企业分发

## 8. 待客户确认问题清单

1. 企业现有 IdP 是什么?是否支持标准 OIDC(授权码 + PKCE)?
2. ~~WorkBuddy 对 §5 四项能力的书面确认~~ ✅ **已通过实测确认**(2026-07-16):前 3 项通过,第 4 项待 UI bug 修复后验证
3. AS 选型(§3 的 A/B/C)
