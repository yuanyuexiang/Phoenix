...
- **生产部署**:打 `v*` 标签触发 GitHub Actions([deploy.yml](.github/workflows/deploy.yml))——
  构建 6 个服务镜像推送阿里云 ACR,SSH 到服务器用 [deploy/docker-compose.prod.yml](deploy/docker-compose.prod.yml)
  滚动更新(Traefik 统一入口:域名 → 管理后台,`/mcp` → 连接器)。服务器 `.env` 参考
  [deploy/.env.prod.example](deploy/.env.prod.example);所需 GitHub Secrets 见 deploy.yml 头部注释。
